...
> ![tips](../common/tips.svg)竅門：搜尋時按下鍵盤上的 **Enter** 鍵是從目前位置向下搜尋，您也可以透過單擊搜尋介面上的 ![up](../common/up.svg) 或 ![down](../common/down.svg)  圖示來向上搜尋或向下搜尋。
...
4. 單擊 **加入其他程式**。
> ![notes](../common/notes.svg)說明：在添加伺服器時，您可以單擊 **進階選項** 來設定更多參數。

![1|addssh](fig/addssh-all.png)


### 編輯伺服器
1.  在終端遠端管理介面上，單擊伺服器上的 ![edit_icon](../common/edit_icon.svg)按鈕。
2.  修改伺服器訊息。
3.  單擊 **儲存**。
...
3. 單擊 **設定** 頁面底部的 **還原預設值** 可以將所有設定項還原到預設狀態。
